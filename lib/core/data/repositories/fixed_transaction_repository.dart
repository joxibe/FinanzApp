...
class FixedTransactionRepository {
  ...
  Future<List<FixedTransaction>> getAllTransactions() async {
    print('Recuperando todas las transacciones fijas');
    final List<Map<String, dynamic>> maps = await _db.query(
      'fixed_transactions',
      orderBy: 'date DESC',
    );
    print('Transacciones fijas encontradas en BD: ${maps.length}');

    final List<FixedTransaction> transactions = [];
    for (var map in maps) {
      print('Procesando transacción fija: $map');
      final categoryMap = await _db.query(
        'fixed_categories',
        where: 'id = ?',
        whereArgs: [map['category_id']],
      );
      print('Categoría fija encontrada: ${categoryMap.isNotEmpty ? categoryMap[0] : "No encontrada"}');
  ...
  Future<void> addTransaction(FixedTransaction transaction) async {
    print('Guardando transacción fija: ${transaction.id} - ${transaction.description}');
    ...
    print('Transacción fija guardada exitosamente');
  }
